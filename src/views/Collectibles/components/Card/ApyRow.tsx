...
import { usePriceTokneBusd } from 'state/hooks'
...
const AprRow: React.FC<AprRowProps> = ({
  ...
  const { stakingToken, totalStaked, isFinished, tokenPerBlock, pid } = pool
  ...
  const earningTokenPrice = usePriceTokneBusd(pid).toNumber() || 0
  ...
  const earningsPercentageToDisplay = () => {
    ...
    return new BigNumber(apr).multipliedBy(1.2).toNumber()
  }
